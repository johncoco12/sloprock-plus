...
const EnvSchema = z.object({
  ...
  SLOPROCK_DEMO_MODE: z.string().transform((v) => v === "1" || v === "true").default(""),
  SLOPROCK_PLUGINS_DIR: z.string().optional(),
  APP_SOURCE_URL: z
    .string()
    .url()
    .default("https://github.com/byrongamatos/sloprock"),
  APP_LICENSE_URL: z.string().url().optional(),
  DATABASE_URL: z.string().optional(),

  SAC_SERVER_NAME: z.string().default("SlopRock"),
  ...
  MINIO_BUCKET: z.string().default("sloprock"),
...
const configDir = env.CONFIG_DIR ?? path.join(homedir(), ".local", "share", "sloprock");
...
export const config = {
  ...
  demoMode: env.SLOPROCK_DEMO_MODE,
  dlcDir: env.DLC_DIR,
  ...
  databaseUrl: env.DATABASE_URL ?? "postgresql://sloprock:sloprock@localhost:5432/sloprock",
  ...
  pluginsUserDir: env.SLOPROCK_PLUGINS_DIR,
