...
export interface HighwayResponse {
  ...
  readonly beats: readonly { time: number; measure: number }[];
  readonly sections: readonly { time: number; name: string }[];
  readonly anchors: readonly WireAnchor[];
  readonly chord_templates: readonly WireChordTemplate[];
  readonly lyrics: readonly { t: number; d: number; w: string }[];
  readonly tone_changes: readonly { time: number; name: string }[];
