...
export class ProfileService {
    ...
    async createProfile(input) {
        ...
        if (existing)
            throw new InvalidCredentialsError("Profile name already exists");
    ...
    async recoverProfile(name, recoveryPhrase, newPin) {
        const profile = await this.profiles.findByName(name);
        if (!profile)
            throw new NotFoundError("Profile");
    ...
    async getPermissions(profileId) {
        ...
        if (!settings || !Array.isArray(settings.permissions))
            return [];
