...
let whichVgmstream;
let whichFfmpeg;
async function resolveVgmstream() {
    if (whichVgmstream !== undefined)
...
async function resolveFfmpeg() {
    if (whichFfmpeg !== undefined)
        return whichFfmpeg;
    const r = await execFileAsync("which", ["ffmpeg"]);
    whichFfmpeg = r.status === 0 ? r.stdout : null;
    return whichFfmpeg;
}
async function decodeWemToWav(vgmstream, wemPath, wavPath) {
...
export class AudioConverterAsync {
    static async convertWem(wemPath, outputBase) {
        const errors = [];
...
//# sourceMappingURL=AudioConverterAsync.js.map