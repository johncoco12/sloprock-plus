...
export const importRoutes = fp(async function importRoutes(fastify) {
    const importService = fastify.imports;
    fastify.post("/api/import/upload", {
        ...
        if (!dlcDir)
            return reply.code(500).send({ error: "DLC_DIR not configured" });
        const providers = fastify.providerRegistry.getAll(IMPORT_FORMAT_PROVIDER_TYPE);
        const results = [];
        for await (const data of parts) {
            const filename = data.filename;
            if (!providers.some((p) => p.canHandle(filename)))
                continue;
