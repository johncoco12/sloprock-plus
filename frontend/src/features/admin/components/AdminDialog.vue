...
<template>
  <AppDialog :open="open" size="xl" @close="emit('close')">
    ...
    <template #header>
      <div class="flex items-center gap-3 min-w-0 flex-1">
        <div class="w-8 h-8 rounded-lg bg-white/[.05] border border-white/[.07] flex items-center justify-center shrink-0">
          <Shield :size="15" class="text-gray-300" />
        </div>
        <h2 class="text-sm font-semibold text-gray-100">{{ $t('admin.title') }}</h2>
      </div>
      <button
        class="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium border transition text-orange-400 border-orange-500/30 bg-orange-500/10 hover:bg-orange-500/20 mr-2"
        :disabled="restarting"
        @click.stop="restartPrompt = true"
      >
    ...
    <div v-if="error" class="mb-4 px-3 py-2 rounded-lg bg-red-500/10 border border-red-500/30 text-red-400 text-sm flex items-center gap-2">
    ...
    <div v-if="activeTab === 'profiles'">
      ...
      <div v-if="showProfileForm" class="fixed inset-0 z-[60] flex items-center justify-center bg-black/60 px-4" @click.self="cancelProfileForm">
        <div class="bg-dark-700 border border-white/[.08] rounded-2xl p-6 w-full max-w-md shadow-xl">
          ...
          <div class="flex justify-end gap-2 mt-6">
            ...
            <button class="settings-btn primary" :disabled="profileSaving" @click="saveProfile">
    ...
    <div v-else-if="activeTab === 'groups'">
      ...
      <div v-if="showGroupForm" class="fixed inset-0 z-[60] flex items-center justify-center bg-black/60 px-4" @click.self="cancelGroupForm">
        <div class="bg-dark-700 border border-white/[.08] rounded-2xl p-6 w-full max-w-lg shadow-xl max-h-[90vh] overflow-y-auto">
          ...
          <div class="flex justify-end gap-2 mt-6">
            ...
            <button class="settings-btn primary" :disabled="groupSaving" @click="saveGroup">
