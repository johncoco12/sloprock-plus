...
Sloppak is Sloprock's open, hand-editable song format. This guide is for developers who want to **read**, **write**, or **extend** the format — including adding new data types like drum tabs, vocal pitches, lighting cues, key/scale annotations, or anything else a future visualization plugin might need.
...
Both forms hold identical contents. Sloprock resolves either transparently — zip files are unpacked to a cache the first time they're opened (see `resolve_source_dir()` in [lib/sloppak.py](../lib/sloppak.py)).
...
Arrangement JSON files use the **wire format** produced by `arrangement_to_wire()` — the on-disk representation of a complete arrangement. Sloprock's `/ws/highway/{filename}` endpoint transports similar data as a sequence of typed messages (`notes`, `chords`, `anchors`, `chord_templates`, `phrases`, …) rather than as one identical top-level JSON object. In practice, the WebSocket stream reuses the same per-object field names where applicable, but it should not be treated as a byte-for-byte match for `arrangements/*.json`.
...
- `displayName` (`string`, default `name`) — label shown in the UI; arrangement XML may use this for display-specific variants such as `-arp`.
...
Sources that carry per-phrase difficulty ladders include this. GP imports and legacy sloppaks omit it:
...
`measure: -1` = sub-beat (not a downbeat). Section `name` follows common conventions (`intro`, `verse`, `chorus`, `bridge`, `solo`, `outro`, …).

### 3.9. Tones (optional)

`tones` carries the arrangement's guitar tones — the amp/pedal/cabinet gear and the in-song tone switches. A sloppak authored from scratch may omit it entirely.
  ...
  "definitions": [
    {
      "Name": "Clean Rhythm",
      "Key": "Tone_A",
      "GearList": { /* gear blocks: Amp, PrePedal1-4, … */ }
    }
  ]
}
```
...
- `definitions` (list) — tone objects (`Name`, `Key`, `GearList`). The Tones plugin parses these into the rendered signal chain (it owns the gear-name/image map, so the data is stored unparsed here).
...
There's no general-purpose writer in `lib/` yet. The reference writer lives in [lib/sloppak_convert.py](../lib/sloppak_convert.py) — it's the single source of truth for "how a sloppak gets built." If you need to write sloppaks from a new source, copy the structure of that function:
...
Older Sloprock readers ignore the unknown `drum_tab` key (the loader uses `manifest.get("drum_tab")` / unknown keys pass through). Your plugin checks for it and renders accordingly. **Zero coordination needed with core.**
...
- **Indexes / IDs**: stable, filesystem-safe, lowercase. Use meaningful string IDs rather than opaque numeric IDs.
...
Or — if your data comes as windows of sustained notes — use this shape:
...
Beats and sections currently live inside the first arrangement JSON for legacy reasons. New song-wide data should be its own file.
...
- **User progress / play counts** — Sloprock stores these in its metadata DB, not in the sloppak.
...
3. **Hand-edit test**: open the directory form in a text editor, change a field by hand, reload Sloprock. The format is meant to be hand-editable; your additions should preserve that.
...
| The reference writer | [lib/sloppak_convert.py](../lib/sloppak_convert.py) |
